...
// note: colors likely don't appear as they should due to sRGB format
...
const CHALLENGE_VERTICES: &[Vertex] = &[
    Vertex { position: [-0.0868241, 0.49240386, 0.0], color: [0.5, 0.0, 0.5] }, // A
    Vertex { position: [-0.49513406, 0.26958647, 0.0], color: [0.5, 0.0, 0.5] }, // B
    Vertex { position: [-0.5513406, 0.06958647, 0.0], color: [0.5, 0.0, 0.5] }, // C
    Vertex { position: [0.1513406, -0.36958647, 0.0], color: [0.5, 0.0, 0.5] }, // D
    Vertex { position: [0.3513406, -0.3158647, 0.0], color: [0.5, 0.0, 0.5] }, // E
    Vertex { position: [0.5513406, 0.0358647, 0.0], color: [0.5, 0.0, 0.5] }, // F
    Vertex { position: [0.5513406, 0.1558647, 0.0], color: [0.5, 0.0, 0.5] }, // G
    Vertex { position: [0.49513406, 0.35958647, 0.0], color: [0.5, 0.0, 0.5] }, // H
];
...
impl State {
    ...
    fn input(&mut self, event: &WindowEvent) -> bool {
        match event {
            WindowEvent::KeyboardInput { input: KeyboardInput {
                ...
                self.toggle_shape = *state == ElementState::Released;
                true
            }
        _ => false,
        }
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        // use encoder to create a RenderPass. RenderPass has all methods for the actual drawing
        {
            ...
            if self.toggle_shape {
                render_pass.set_vertex_buffer(0, self.challenge_vertex_buffer.slice(..)); // .. specifies entire buffer; can store as many objects in a buffer as hardware allows
                render_pass.set_index_buffer(self.challenge_index_buffer.slice(..), wgpu::IndexFormat::Uint16);
                render_pass.draw_indexed(0..self.challenge_num_indices, 0, 0..1); // draw something with 3 vertices and 1 instance, this is where @builtin(vertex_index) comes from
            } else {
